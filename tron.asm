...
PLAYER1_CHANGEDIR_RIGHT:
	...
	PUSH AF
	PUSH HL
	LD   HL,PLAYER1_MOVEDIR			; load current player1_movedir into HL
	LD   BC,3				; load new movedir into BC (RIGHT)
	LD   A,2				; load movedir left into A (2)
	CP   (HL)				; check if player1_movedir is left
	JR   Z,P1_SKIP_CHANGEDIR_RIGHT		; if player1_movedir is left, he can't start moving right - so skip changing direction
	LD   (PLAYER1_MOVEDIR),BC		; change player1 movedir to right
...
PLAYER1_CHANGEDIR_UP:
	...
	CP   (HL)				; check if player1_movedir is 0 (down)
	JR   Z,P1_SKIP_CHANGEDIR_UP		; if player1_movedir is down, he can't start moving up - so skip changing direction
	LD   (PLAYER1_MOVEDIR),BC		; change player1 movedir to 1 (up)
...
PLAYER1_CHANGEDIR_DOWN:
	...
	PUSH HL
	LD   HL,PLAYER1_MOVEDIR			; load current player1_movedir into HL
	LD   BC,0				; load new movedir into BC (DOWN)
	LD   A,1				; load movedir UP into A (1)
	CP   (HL)				; check if player1_movedir is UP
	JR   Z,P1_SKIP_CHANGEDIR_DOWN		; if player1_movedir is up, he can't start moving down - so skip changing direction
	LD   (PLAYER1_MOVEDIR),BC		; change player1 movedir to DOWN
...
PLAYER1_CHANGEDIR_LEFT:
	...
	PUSH HL
	LD   HL,PLAYER1_MOVEDIR			; load current player1_movedir into HL
	LD   BC,2				; load new movedir into BC (LEFT)
	LD   A,3				; load movedir RIGHT into A (3)
	CP   (HL)				; check if player1_movedir is right
	JR   Z,P1_SKIP_CHANGEDIR_LEFT		; if player1_movedir is right, he can't start moving left - so skip changing direction
	LD   (PLAYER1_MOVEDIR),BC		; change player1 movedir to left (2)
...
PLAYER2_CHANGEDIR_RIGHT:
	...
	LD   BC,3				; load new movedir into BC (RIGHT)
	LD   A,2				; load movedir left into A (2)
	CP   (HL)				; check if player2_movedir is left
	JR   Z,P2_SKIP_CHANGEDIR_RIGHT		; if player2_movedir is left, he can't start moving right - so skip changing direction
	LD   (PLAYER2_MOVEDIR),BC		; change player2 movedir to right (3)
...
PLAYER2_CHANGEDIR_DOWN:
	...
	LD   BC,0				; load new movedir into BC (DOWN)
	LD   A,1				; load movedir up into A (1)
	CP   (HL)				; check if player2_movedir is up
	JR   Z,P2_SKIP_CHANGEDIR_DOWN		; if player2_movedir is up, he can't start moving down - so skip changing direction
	LD   (PLAYER2_MOVEDIR),BC		; change player2 movedir to 0 (down)
...
PLAYER2_CHANGEDIR_LEFT:
	...
	LD   BC,2				; load new movedir into BC (LEFT)
	LD   A,3				; load movedir right into A (3)
	CP   (HL)				; check if player2_movedir is right
	JR   Z,P2_SKIP_CHANGEDIR_LEFT		; if player2_movedir is right, he can't start moving uleft - so skip changing direction
	LD   (PLAYER2_MOVEDIR),BC		; change player2 movedir to 2 (left)
...
MOVE_PLAYER2_DOWN:
	...
	PUSH HL
	PUSH DE
	LD   BC,$0020				; load increment value into BC
	LD   HL,(PLAYER2_POS)			; load player2 pos (current attribute block address) into HL
...
MOVE_PLAYER2_LEFT:
	...
	PUSH HL
	PUSH DE
	LD   BC,$FFFF				; load increment value into BC
	LD   HL,(PLAYER2_POS)			; load player2 pos (current attribute block address) into HL
	ADD  HL,BC				; add increment value to player2 pos (go right 1 screen block address)
	LD   A,(HL)				; next screen block address into A
	LD   HL,(PLAYER2_POS)			; load player2 pos (current attribute block address) into HL
...
MOVE_PLAYER1_UP:
	...
	PUSH HL
	PUSH DE
	LD   BC,$FFE0				; load increment value into BC
	LD   HL,(PLAYER1_POS)			; load player1 pos (current attribute block address) into HL
	ADD  HL,BC				; add increment value to player1 pos (go right 1 screen block address)
	LD   A,(HL)				; next screen block address into A
	LD   HL,(PLAYER1_POS)			; load player1 pos (current attribute block address) into HL
	CP   $47				; check if next position of player1 is in wall
	CALL NZ,PLAYER2_WINS			; if YES, end game with player2 wins
	LD   BC,$FFE0				; load increment value into BC
	LD   (HL),$68				; load color "bright yellow" into HL -> previous player1 pos
	ADD  HL,BC				; calculate new player1 pos
	LD   (HL),$48				; load color "bright red" into HL -> new player1 pos
	LD   (PLAYER1_POS),HL			; store new player1 pos into memory
	POP  DE
	POP  HL
	POP  BC
	POP  AF
...
MOVE_PLAYER1_DOWN:
	...
	LD   A,(HL)				; next screen block address into A
	LD   HL,(PLAYER1_POS)			; load player1 pos (current attribute block address) into HL
	CP   %01000111				; check if next position of player1 is in wall
	CALL NZ,PLAYER2_WINS			; if YES, end game with player2 wins
	LD   BC,$0020				; load increment value into BC
	LD   (HL),$68				; load color "bright yellow" into HL -> previous player1 pos
	ADD  HL,BC				; calculate new player1 pos
...
MOVE_PLAYER1_RIGHT:
	...
	PUSH HL
	PUSH DE
	LD   BC,$0001				; load increment value into BC
	LD   HL,(PLAYER1_POS)			; load player1 pos (current attribute block address) into HL
	ADD  HL,BC				; add increment value to player1 pos (go right 1 screen block address)
	LD   A,(HL)				; next screen block address into A
	LD   HL,(PLAYER1_POS)			; load player1 pos (current attribute block address) into HL
...
SET_BORDER_COLOR:
	...
	CALL $229B			; call routine from ROM to change border color
	RET 
...
WINNING_PLAYER:
	db 0
BORDER_BITMAP_POINTER:			; i have no idea why this isn't just a pointer to the border bitmap
	db $00
	db $84
	db $00 
	db $58 
